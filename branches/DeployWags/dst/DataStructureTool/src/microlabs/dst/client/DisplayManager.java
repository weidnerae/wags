...
{
	//For RPC
	private final ProblemResultStoreServiceAsync probResStoreService = GWT
	.create(ProblemResultStoreService.class);
	...
	public DisplayManager(DrawingArea canvas, AbsolutePanel panel,
			NodeCollection nc, EdgeCollection ec, Problem problem, int attemptNum)
	{
	...
	private void addEvaluateButton()
	{
		...
		evaluateButton.addClickHandler(new ClickHandler()
		{
			public void onClick(ClickEvent event)
			{
				...
				ProblemResult result = new ProblemResult(problem.getName(), problem.getProblemText(), serNodes, serEdges, evalResult, attemptNum++);
				probResStoreService.saveProblemResults(result, new AsyncCallback<String>() 
						{
