...
public class Evaluation_HeapSort extends Evaluation implements IsSerializable{
	...
	public String evaluate(String problemName, String[] arguments,ArrayList<Node> nodes, ArrayList<EdgeParent> edges) {
		...
		if(solution.equals(cSolution)){
			...
			if(correctSection.equals("")){
				...
				return "Feedback: Incorrect. Hint, the first item is "+splitCSolution[0];
			}
			return "Feedback: Incorrect. You were correct for the section: "+correctSection;
		}
	}
    ...
    public void immobilizeNode(ArrayList<Node> nodes,String solution){
    	...
    	for(int i=nodes.size()-1;i>=0;i--){
    		if(nodes.get(i).getValue().equals(desiredNode)){
    			...
    			else if(nodes.get(i).getLeft()>nodes.get(index).getLeft() && !(nodes.get(i).getLabel().getStyleName().equals("immobilized_node"))){
    				index=i;
    			}
    		}
    	}
		NodeDragController.getInstance().makeNotDraggable(nodes.get(index).getLabel());
	...
	public void updateCounterPanel(){
    	if (RootPanel.get().getWidget(2) instanceof TextArea){
    		((TextArea) RootPanel.get().getWidget(2)).setText("Current Pass: "+CURRENT_STEP);
		}
	}
